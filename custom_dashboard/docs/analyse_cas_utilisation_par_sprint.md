...
```plantuml
@startuml
class "Responsable métier" as Responsable {
  identifiant
  domaine métier
}
class "Administrateur système" as Admin {
  identifiant
  rôle
}
class "Rôle fonctionnel" as Role {
  nom
  domaine
}
class "Fichier importé" as Fichier {
  nom
  format
  taille
  date d’importation
}
class "Import de données" as Import {
  statut
  feuille sélectionnée
  ligne d’en-tête
}
class "Prévisualisation des données" as Apercu {
  colonnes détectées
  premières lignes
  nombre de lignes
}
class "Jeu de données" as Jeu {
  nom
  description
  statut
  nombre de lignes
...
class "Tableau de bord" as Tableau {
  titre
  domaine
  état
  visibilité
}
class "Droit d’accès au widget" as DWidget {
  consultation autorisée
  utilisation autorisée
}
class "Droit d’accès au tableau de bord" as DTableau {
  consultation autorisée
  édition autorisée
  type de partage
}
class "Référentiel décisionnel" as Referentiel {
  données préparées
  permissions
}
...
Responsable "*" -- "*" Role
Admin "*" -- "*" Role
Responsable "1" -- "*" Import : réalise
Fichier "1" -- "1" Import
Import "1" *-- "0..1" Apercu
Import "0..1" --> "1" Jeu : prépare
Referentiel "1" o-- "*" Jeu
Referentiel "1" o-- "*" Widget
Referentiel "1" o-- "*" Tableau
Role "1" -- "*" DWidget
DWidget "*" --> "1" Widget
Role "1" -- "*" DTableau
DTableau "*" --> "1" Tableau
Admin ..> DWidget : configure
Admin ..> DTableau : configure
@enduml
```
...
class "Gestionnaire des tableaux de bord" as Gestionnaire {
  identifiant
  rôle
}
class "Tableau de bord" as Tableau {
...
class "Élément de tableau de bord" as Element {
...
class "Catalogue des widgets" as Catalogue {
  domaines
  widgets disponibles
}
class "Filtre de widget" as Filtre {
  nom
  type
  valeur par défaut
}
class "Résultat de visualisation" as Resultat {
  type d’affichage
  valeur synthétique
  données présentées
}
class "Paramètre visuel" as Parametre {
  libellé
  valeur
}
class "Droit d’accès" as Droit {
  consultation autorisée
  utilisation autorisée
}
class "Constructeur de tableaux de bord" as Constructeur {
  création d’une vue
  organisation d’une vue
  sauvegarde d’une vue
}

Gestionnaire ..> Constructeur : utilise
Constructeur ..> Tableau : crée et modifie
Catalogue "1" o-- "*" Widget
Tableau "1" *-- "*" Element
Element "*" --> "1" Widget
Element "1" o-- "*" Parametre
Widget "1" o-- "*" Filtre
Widget "1" --> "0..1" Resultat
Gestionnaire "*" -- "*" Droit
Responsable "*" -- "*" Droit
Droit "*" --> "*" Widget
Gestionnaire "1" -- "*" Tableau : crée
Responsable "*" -- "*" Tableau : consulte
@enduml
```
...
class "Question métier" as Question {
  contenu
  domaine identifié
}
class "Réponse IA" as Reponse {
  contenu
  niveau de confiance
}
class "Analyse assistée" as Analyse {
  nom
  résumé
  tendances
  recommandations
...
class "Retour utilisateur" as Retour {
  appréciation
  commentaire
  réponse attendue
}
...
Responsable "1" -- "*" Conversation
Conversation "1" *-- "*" Message
Message <|-- Question
Message <|-- Reponse
Conversation "1" o-- "*" Retour
Retour "*" --> "0..1" Reponse
Responsable "1" -- "*" Droit
Question --> Droit : nécessite vérification
Analyse "*" --> "0..1" Tableau
Analyse "*" --> "0..1" Jeu
Responsable "1" -- "*" Analyse
Analyse "0..1" --> "*" Retour
Agent ..> Question : interprète
Agent ..> Reponse : produit
Agent ..> Analyse : assiste
@enduml
```
